...
function Navbar({ page, setPage }) {
  ...
  const toast = useToast();
  const links = [
    { key: "dashboard", icon: "📊", label: "Dashboard", roles: ["MERCHANT"] },
    ...
    <div className="page-enter">
      ...
      <Modal open={paymentModal} onClose={() => setPaymentModal(false)} title="💳 Tagihan Pembayaran">
          ...
          <div style={{ background: C.offWhite, borderRadius: 16, padding: "20px", marginBottom: 16, border: `1px solid ${C.gray100}` }}>
            ...
            {paymentOrders[0]?.transaksi?.metode_bayar === "TRANSFER" && (
              <div>
                ...
                <div style={{ fontSize: 13, color: C.gray600, textAlign: "left", background: C.white, padding: "16px", borderRadius: 10, border: `1px solid ${C.blueLight}` }}>
                  <strong style={{ display: "block", marginBottom: 8 }}>Cara Bayar:</strong>
                  1. Lakukan transfer ke rekening di atas sebesar <strong>Rp{paymentOrders.reduce((sum, o) => sum + Number(o.transaksi?.total_bayar || 0), 0).toLocaleString("id-ID")}</strong>.<br />
                  2. Pastikan nama penerima adalah "Kantin Telkom Pusat".<br />
                  3. Simpan dan tunjukkan bukti transfer ke admin/kasir pusat untuk diverifikasi.
                </div>
              </div>
            )}
          </div>
        ...
        <div style={{ marginTop: 20 }}>
          <BtnRed full onClick={() => setPaymentModal(false)}>Selesai</BtnRed>
        </div>
      </Modal>
    </div>
  );
...
function AdminPage() {
  ...
  const lunasTransaksi = async id => {
    ...
    try { await apiFetch("PUT", `/admin/transaksi/${id}/lunas`, null, token); toast("Pembayaran berhasil dikonfirmasi! ✅"); load(); }
    catch (e) { toast(e.message, "error"); }
  };
...
function AppCore() {
  const { user } = useAuth();
  const [page, setPage] = useState(user?.role === "MERCHANT" ? "dashboard" : "menu");
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.offWhite }}>
      ...
      <main style={{ maxWidth: 1100, margin: "0 auto", padding: mobile ? "20px 16px 90px" : "28px 20px" }}>
        ...
        {page === "dashboard" && user?.role === "MERCHANT" && <MerchantPage />}
